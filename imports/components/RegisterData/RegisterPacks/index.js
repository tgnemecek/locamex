...
class RegisterPacks extends React.Component {
  ...
  onChangePlace = (e) => {
    ...
    var place =
      this.props.placesDatabase.find((item) => {
        return item._id === _id;
      }) || {};
    this.setState({ place });
  };
  ...
  saveEdits = () => {
    ...
    Meteor.call("packs.update", this.props.item._id, data, (err, res) => {
  ...
  render() {
    return (
      <Box
        className="register-data register-packs"
      ...
      >
        <div className="register-packs__information">
          ...
          <Input
            title="Pátio:"
            type="select"
            disabled={this.props.item.rented || !tools.isWriteAllowed("packs")}
            value={this.state.place._id}
            onChange={this.onChangePlace}
          >
            {this.props.item.rented && <option value="">Alugado</option>}
            {this.renderPlaces()}
          </Input>
          <Input
            title="Observações:"
              ...
              ? [
                  {
                    ...
                    onClick: () =>
                      this.props.toggleConfirmationWindow(this.unmountPack, {
