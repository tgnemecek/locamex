...
export default class MainHeader extends React.Component {
  ...
  checkIfHasContent = () => {
    var hasContent =
      this.props.snapshot.observations.internal ||
      this.props.master.observations.external;
  ...
  snapshotsOptions = () => {
    ...
    for (var i = 0; i < snapshots.length; i++) {
      ...
      result.unshift(
        <option key={i} value={i} style={style}>
          {i + 1}
        </option>
      );
    }
          ...
          <div className="main-header__title">
            ...
            {this.props.snapshots && this.props.snapshots.length ? (
              this.props.changeSnapshot ? (
                ...
                <h1>.{this.props.snapshotIndex + 1}</h1>
              )
            ) : null}
          </div>
        </div>
        <div className="main-header__right-side">
          {this.props.toggleBilling ? (
            ...
            >
              <Icon
                icon="money"
                style={this.props.billingError ? { color: "red" } : null}
              />
            </button>
          ) : null}
          {this.props.toggleDocuments ? (
            ...
            >
              <Icon
                icon="print"
                style={this.props.documentsError ? { color: "red" } : null}
              />
            </button>
          ) : null}
          ...
          <h3>
            Status: <Status status={this.props.status} type={this.props.type} />
          </h3>
        </div>
        <ConfirmationWindow
