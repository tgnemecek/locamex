...
export default class BuySellRent extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      in: 0,
    ...
    return (
      <div className="stock-visualizer__buy-sell-wrapper">
        <div className="stock-visualizer__buy-sell-ammounts" style={this.props.errorKeys.includes("sum") ? {color: "red"} : null}>
          {this.props.shippingMode ? "LOCANDO" : "TOTAL"}: {this.props.totalItems} / SOMA ATUAL: {this.props.sumItems}
        </div>
        {this.props.shippingMode ? null :
          <>
