...
export default class DocumentsCompany extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      negociatorId: this.props.proposal.negociatorId || '',
      representativesId: this.props.proposal.representativesId || [],
  ...
  displayContacts = (which) => {
    var contacts = [];
    if (which === 'negociator') {
      contacts = this.props.proposal.client.contacts.filter((contact) => {
  ...
  generate = () => {
    ...
    const getPersonUsingId = (arrayOfIds) => {
      ...
      arrayOfIds.forEach((id) => {
        this.props.proposal.client.contacts.forEach((item) => {
          if (item._id === id) {
            people.push(item)
          }
        })
    ...
    this.props.updateProposal({
      ...
      createPdf({ ...this.props.proposal, negociator, representatives });
