...
export default class PlacesDistribution extends React.Component {
  ...
  return places.map((place, i) => {
    return (
      <div key={i}>
        <span className="shipping-accessories__select-multiple__place">
          <span>{this.getDescriptionPlace(place._id)}</span>
          <span>{place.available}</span>
        </span>
      </div>
    );
  })
    ...
    // var placesDatabase = this.props.placesDatabase.filter((place) => {
    //
    // });
    //
