...
export default class BillingSchedule extends React.Component {
  ...
  setBillingProducts = () => {
    ...
    var calculation = this.getEqualValues(
      this.props.totalValue('products'), duration
    );
  ...
  setBillingServices = (duration) => {
    if (duration === 1 &&
        this.props.snapshot.billingServices.length) {
          return;
      }
    ...
    var billingServices = [];
    var calculation = this.getEqualValues(
      this.props.totalValue('services'), duration
    )
    for (var i = 0; i < duration; i++) {
  ...
  saveEdits = () => {
    var allBills = this.state.billingProducts
    ...
    const hasInvalidDifference = () => {
      ...
      return this.props.totalValue() - total !== 0;
    }
