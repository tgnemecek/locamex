...
export default class tools {
  ...
  static totalValue = (snapshot) => {
    var duration =
      snapshot.dates.timeUnit === "months" ? snapshot.dates.duration : 1;
  ...
  static filterSearch = (database, state) => {
    const compare = (a, b) => {
      a = tools
        .removeSpecialChars(a.toString(), /[\.\/\-\(\) ]/g)
        .toUpperCase();
      b = tools
        .removeSpecialChars(b.toString(), /[\.\/\-\(\) ]/g)
        .toUpperCase();
    ...
    if (state.filter) {
      var filterArray = Array.isArray(state.filter)
        ? state.filter
        : [state.filter];
  ...
  static findSubstring = (substring, stringArray) => {
    ...
    substring = tools
      .removeSpecialChars(substring, /[\.\/\-\(\) ]/g)
      .toUpperCase();

    for (var i = 0; i < stringArray.length; i++) {
      stringArray[i] = tools
        .removeSpecialChars(stringArray[i], /[\.\/\-\(\) ]/g)
        .toUpperCase();
  ...
  static translateError = (error) => {
    ...
    var dictionary = {
      ...
      "duplicate-registry":
        "O Cliente que você está tentando cadastrar já existe.",
      "description-in-use": "Série já existente. Favor escolher outra.",
      "stock-must-be-zero":
        "Para exclusão, o estoque deve ser zero, incluindo locações em andamento.",
      "item-must-not-be-rented": "O item a ser excluído não pode estar locado.",
      "active-contract-using-item":
        "Para exclusão, nenhum contrato ativo pode conter o item a ser excluído. Contrato: " +
        error.reason,
      "document-contains-deleted-items":
        "O documento não pode ser ativado pois contém itens deletados. Item: " +
        error.reason,
    };
  ...
  static checkCpf(arg) {
    ...
    sum = 0;
    for (i = 1; i <= 10; i++)
      sum = sum + parseInt(arg.substring(i - 1, i)) * (12 - i);
    remainder = (sum * 10) % 11;
