...
export default function billingPdf({
  contract, snapshot, bill, header, generateTable, styles}) {
    // const contract = props.master;
    // const generateTable = props.generateTable;
    // const styles = props.styles;
    // const bill = props.bill;

    const products = snapshot.containers.concat(snapshot.accessories)
      .map((item) => {
    ...
    }, 0) * (1 - snapshot.discount)) : 0;

    function resultFormat(input) {
      return {text: tools.format(input, 'currency'), alignment: 'right', bold: true};
    }

    var activeVersion = contract.snapshots.findIndex((snapshot) => {
      return snapshot.active;
    }) + 1;

    return {
      fileName: `Locamex - Contrato #${contract._id}_${activeVersion} - Fatura ${bill.index+1} de ${bill.length}.pdf`,
      pageSize: 'A4',
      pageMargins: [ 40, 110, 40, 45 ], //[left, top, right, bottom]
      info: {
        title: `Fatura ${bill.index+1} de ${bill.length} - Locamex #${contract._id}.${activeVersion}`,
        author: `Locamex`,
      ...
      footerStatic: `Locamex - Contrato #${contract._id}.${activeVersion} - Fatura ${bill.index+1} de ${bill.length}\n`,
      styles
    };
}