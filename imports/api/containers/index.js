...
if (Meteor.isServer) {
  ...
  function updateReferences(_id, changes, dbNames) {
    let Databases = {
      Series,
      Proposals,
      Contracts
    ...
    dbNames.forEach((dbName) => {
      if (dbName === "Series") {
        Series.update({ "container._id": _id }, {$set: {
          "container.description": changes.description
        }}, {multi: true})
      } else {
        let Database = Databases[dbName];
        Database.find({ $and: [{status: "inactive", "snapshots.containers._id": _id}] })
        .forEach((doc) => {
            ...
            Database.update({ _id: doc._id }, {$set: doc});
        })
      }
    })
  }
